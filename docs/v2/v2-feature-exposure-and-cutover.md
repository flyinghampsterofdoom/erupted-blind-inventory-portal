...
| `ordering_intelligence_v2` | V2 Ordering Intelligence dashboard plus lifecycle management surfaces | Yes | Yes | Recommendation viewing requires effective `management.admin` and server-resolved authorized stores. Lifecycle mutation additionally requires explicit `ordering.lifecycle.manage`, CSRF, and optimistic versions. Reads remain synchronous; no PO or Square write is introduced. |
