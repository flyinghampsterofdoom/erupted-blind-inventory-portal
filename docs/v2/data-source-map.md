...
| Funding reports | SKU and base product identity | External authoritative read model | Square catalog | `ordering_catalog_identity` | Existing Square catalog refresh | Funding SKU mappings and report lines | Exact normalized SKU; variation ID retained only for traceability | Read only | Never matches by product name or partial SKU | Verified locally and on PostgreSQL |
| Funding reports | Completed sales and itemized returns | External transaction facts | Square | `consignment_sale_facts`, `consignment_return_facts` | Existing explicit Square synchronization | Manual Consignment and Credit Card period reports | Saved fact links, transaction IDs, line UIDs, dates, quantities | V2 report snapshots only | Selected dates control inclusion; overlap never silently excludes facts | Verified locally and on PostgreSQL |
| Funding reports | Current quantity on hand | External current-state read model | Square inventory | `ordering_current_inventory` | Existing explicit inventory refresh | Account and report inventory estimates | SKU-to-variation match and store snapshot timestamp | Read only | Never used to infer sales or lot lineage | Verified locally and on PostgreSQL |
| Funding reports | Consignment eligibility, saved cost, financial vendor, and original vendor | Internal authoritative fact | Version 1 ordering plus V2 financial assignment | `purchase_orders`, `purchase_order_lines`, `order_payments` | Existing V1 lifecycle and explicit owner assignment/reassignment | Consignment report eligibility and source summary | Current financial vendor from `order_payments.vendor_id`; original vendor, order, line, SKU, quantity, and cost snapshot preserved | No V1 write | No assigned qualifying order or usable SKU fails closed; original vendor and global mappings never establish eligibility | Verified by regression and non-mutation tests |
| Funding reports | Credit Card vendor scope and effective SKU-to-account cost history | Internal authoritative assignment plus explicit owner override | V2 | Active `vendors` linked through `vendor_payment_settings` to the account payment method; `funding_accounts`, `funding_sku_mappings` | Square vendor synchronization, owner payment-method assignment, and owner-only mapping action | Credit Card report and payment workflows | Report/payment vendor ID, exact normalized SKU, effective dates, and vendor-attributed sale/return facts | V2 only | Vendor selection is required; inactive, unconfigured, cross-vendor, or ambiguous legacy scope fails closed | Verified locally; production verification required |
| Funding reports | Saved calculations and source history | Internal authoritative fact | V2 | `funding_reports`, lines, fact links, exclusions, adjustments | Owner calculates, finalizes, adjusts, reverses, or voids | Report preview and history | Snapshot IDs, mappings, facts, actor, timestamps, overlap acknowledgement | V2 only | Finalized values are not regenerated from current mappings | Verified locally and on PostgreSQL |
| Funding reports | Payments, replenishment, card balance, and APR terms | Explicit owner-entered financial record | V2 | `funding_payments`, allocations, `funding_ledger_entries`, `funding_accounts` APR fields | Owner-only typed actions | Account detail, report history, balance and estimate views | Append-only payment/ledger/reversal links | V2 only | No automatic payment; APR output is an estimate, actual interest is explicit | Verified locally and on PostgreSQL |
...
- `V2_CREDIT_CARD_COGS_ACTIONS_ENABLED=false` independently server-blocks Credit Card funding-account,
  report, APR, payment, and ledger mutations. Both gates remain inside principal-scoped `order_payments_v2`.
