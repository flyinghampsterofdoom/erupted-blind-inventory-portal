...
| Consignment settlement | Financial treatment/replenishment identity | Internal authoritative fact | V2 linked to V1 | `order_payments`, `consignment_replenishments` | `ensure_order_payment` from vendor default at initialization | V2 payment and consignment pages | Method/treatment/order value frozen | V2 only | Consignment never uses PAID; missing costs block received valuation | Verified |
