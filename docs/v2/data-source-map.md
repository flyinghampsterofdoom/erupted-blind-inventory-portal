...
| Funding reports | Credit Card effective SKU-to-account cost history | Explicit owner override | V2 | `funding_accounts`, `funding_sku_mappings` | Owner-only mapping action | Credit Card report calculation | Exact normalized SKU plus effective dates | V2 only | Conflicts, missing mappings, and missing costs block or exclude unsafe attribution | Verified locally and on PostgreSQL |
