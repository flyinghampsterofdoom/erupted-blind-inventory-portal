...
| Funding reports | Original purchase order, saved cost, receipt, financial vendor, and payment method | Internal authoritative fact | Version 1 ordering plus V2 financial assignment | V1 order tables; `order_payments` | Existing V1 lifecycle and explicit owner assignment | Purchase funding ledger link only | `order_payment_id`; original V1 order remains unchanged | No V1 write | Square customer orders are never treated as internal purchase orders | Verified by non-mutation tests |
| Funding reports | Effective SKU-to-account cost history | Explicit owner override | V2 | `funding_accounts`, `funding_sku_mappings` | Owner-only mapping action | Report calculation | Exact normalized SKU plus effective dates | V2 only | Conflicts, missing mappings, and missing costs remain visible exclusions | Verified locally and on PostgreSQL |
